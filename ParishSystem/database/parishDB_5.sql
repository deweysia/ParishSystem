...
-- -----------------------------------------------------
-- Table `sad2`.`blooddonor`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `sad2`.`blooddonor` (
  `bloodDonorID` INT NOT NULL AUTO_INCREMENT,
  `profileID` INT NOT NULL,
  `bloodType` CHAR(5) NULL,
  PRIMARY KEY (`bloodDonorID`),
  INDEX `bloodDonorID_profileID_idx` (`profileID` ASC),
  CONSTRAINT `bloodDonorID_profileID`
    FOREIGN KEY (`profileID`)
    REFERENCES `sad2`.`generalprofile` (`profileID`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
...
CREATE TABLE IF NOT EXISTS `sad2`.`blooddonation` (
  ...
  `bloodDonationDateTime` DATETIME NULL,
  ...
  INDEX `bloodDonation_bloodDonor_idx` (`bloodDonorID` ASC),
  ...
  CONSTRAINT `bloodDonation_bloodDonor`
    FOREIGN KEY (`bloodDonorID`)
    REFERENCES `sad2`.`blooddonor` (`bloodDonorID`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB
DEFAULT CHARACTER SET = utf8;
...
CREATE TABLE IF NOT EXISTS `sad2`.`marriage` (
  ...
  INDEX `marriage_app_idx` (`applicationID` ASC),
  INDEX `marriage_minister_idx` (`ministerID` ASC),
  CONSTRAINT `marriage_app`
    FOREIGN KEY (`applicationID`)
    REFERENCES `sad2`.`application` (`applicationID`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
...
CREATE TABLE IF NOT EXISTS `sad2`.`sponsor` (
  ...
  `suffix` VARCHAR(10) NULL DEFAULT NULL,
  `gender` CHAR(1) NULL DEFAULT NULL,
  `sacramentType` VARCHAR(45) NULL,
  `residence` VARCHAR(45) NULL,
  PRIMARY KEY (`sponsorID`),
  INDEX `sacrament_baptism_idx` (`sacramentID` ASC),
  CONSTRAINT `sacrament_baptism`
    FOREIGN KEY (`sacramentID`)
    REFERENCES `sad2`.`baptism` (`applicationID`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `sacrament_confirmation`
    FOREIGN KEY (`sacramentID`)
    REFERENCES `sad2`.`confirmation` (`applicationID`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `sacrament_marriage`
    FOREIGN KEY (`sacramentID`)
    REFERENCES `sad2`.`marriage` (`marriageID`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB
DEFAULT CHARACTER SET = utf8;
...
CREATE TABLE IF NOT EXISTS `sad2`.`income` (
  ...
  INDEX `income_genprof_idx` (`sourceID` ASC),
  CONSTRAINT `income_genprof`
    FOREIGN KEY (`sourceID`)
    REFERENCES `sad2`.`generalprofile` (`profileID`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `income_incomeSource`
    FOREIGN KEY (`sourceID`)
    REFERENCES `sad2`.`incomeSource` (`incomeSourceID`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
